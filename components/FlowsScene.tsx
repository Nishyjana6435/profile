...
/** Workflow nodes floating around the screenshot, at different depths. */
const CHIPS: Chip[] = [
  {
    sub: "Trigger",
...
export function FlowsLogo({ className = "" }: { className?: string }) {
  return (
    <svg viewBox="0 0 32 32" fill="none" aria-hidden="true" className={className}>
      ...
      <path
        d="M11.5 14.5 20.5 10.2M11.5 17.5l9 4.3"
        stroke="#121317"
        strokeWidth="2"
        strokeLinecap="round"
      />
    ...
    >
      <div className="fx-stage absolute inset-0">
        <div className="fx-sway absolute inset-0">
          ...
          <div className="fx-frame absolute inset-0 overflow-hidden rounded-2xl border border-white/15 bg-[#0d0d10] shadow-[0_70px_160px_-50px_rgba(139,92,246,0.75)]">
            <div className="flex h-9 items-center gap-2 border-b border-white/10 bg-[#16161a] px-3.5">
              ...
              <span className="ml-3 flex h-5 flex-1 items-center justify-center rounded-md bg-white/[0.06] text-[10px] text-white/50 ring-1 ring-white/10">
                ...
                <span className="truncate px-1">{FLOWS.host}</span>
              </span>
            </div>
            <div className="relative h-[calc(100%-2.25rem)] w-full">
              <Image
                src="/flows/hero.webp"
                alt={`${FLOWS.name} landing page: “${FLOWS.tagline}”`}
                fill
                priority={false}
          ...
          {/* animated wires between the workflow nodes */}
          <svg
            aria-hidden="true"
            viewBox="0 0 100 100"
          ...
          {/* prompt panel, mid depth */}
          <div
            className="fx-layer absolute left-0 -bottom-[6%] w-[46%] overflow-hidden rounded-xl border border-white/15 bg-[#121317] shadow-[0_30px_80px_-24px_rgba(0,0,0,0.9)] sm:-left-[6%]"
            style={{ "--z": "70px", "--m": 1.2, "--d": "0.1s" } as CSSProperties}
          >
            <Image
              src="/flows/prompt.webp"
              alt="Flows prompt panel: describe what should happen in plain English"
              width={486}
              height={325}
          ...
          >
            <Image
              src="/flows/workflow.webp"
              alt="Flows workflow panel: trigger, steps and a successful test run"
              width={536}
              height={325}
